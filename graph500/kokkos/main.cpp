...
int main(int argc, char** argv) {
  ...
  for (bfs_root_idx = 0; bfs_root_idx < num_bfs_roots; ++bfs_root_idx) {
...
#ifdef DEBUG
    printf("Time spent in BFS %d was %g s\n", bfs_root_idx, bfs_times[bfs_root_idx]);
#endif

    /* Validate result. */
    //if (rank == 0) fprintf(stderr, "Validating BFS %d, nvisted = %ld\n", bfs_root_idx, nvisited);

    double validate_start = MPI_Wtime();
    int validation_passed_one = 1; //validate_bfs_result(&g, root, pred/*, nvisited*/);
    ...
    /* Calculate number of input edges visited. */
    {
      ...
      {
          int64_t edge_visit_count_tmp = edge_visit_count;
          MPI_Allreduce(&edge_visit_count_tmp, &edge_visit_count, 1, INT64_T_MPI_TYPE, MPI_SUM, MPI_COMM_WORLD);
      }
      edge_counts[bfs_root_idx] = (double)edge_visit_count;
    }
    /*
    Kokkos::host_spawn(Kokkos::TaskSingle(sched, Kokkos::TaskPriority::High),
        task_functor_calc_input_edgs());
    */
    //MPI_Abort(MPI_COMM_WORLD, 1);
  }
...
//  MPI_Free_mem(pred);
  ...
  if (rank == 0) {
    if (!validation_passed) {
      ...
      for(i=0; i < num_bfs_roots; ++i) total_bfs_time += bfs_times[i];
      fprintf(stdout, "Total time for BFS:             %g s\n", total_bfs_time);
#if 0
