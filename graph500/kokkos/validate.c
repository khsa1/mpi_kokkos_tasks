...
int validate_bfs_result(const csr_graph* const g, const int64_t root, const int64_t* const pred) {
  ...
  {
      int any_range_errors_tmp = any_range_errors;
      MPI_Allreduce(&any_range_errors_tmp, &any_range_errors, 1, MPI_INT, MPI_LOR, MPI_COMM_WORLD);
  }
  ...
  {
      int64_t nvisited_actual_tmp = nvisited_actual;
      MPI_Allreduce(&nvisited_actual_tmp, &nvisited_actual, 1, INT64_T_MPI_TYPE, MPI_SUM, MPI_COMM_WORLD);
  }
  ...
  if (!any_range_errors) { /* Other parts of validation assume in-range values */
    ...
    { /* Scope some code that has a lot of temporary variables. */
      ...
      while (1) {
        ...
        {
            int any_changed_tmp = any_changed;
            MPI_Allreduce(&any_changed_tmp, &any_changed, 1, MPI_INT, MPI_LOR, MPI_COMM_WORLD);
        }
  ...
  {
      int validation_passed_tmp = validation_passed;
      MPI_Allreduce(&validation_passed_tmp, &validation_passed, 1, MPI_INT, MPI_LAND, MPI_COMM_WORLD);
  }
